...
class GrantConfiguredModeratorsUseCaseTest {
    ...
    void deberia_otorgar_el_rol_de_moderador_a_la_cuenta_configurada() {
        User quien = cuenta("moderadora@sendik.co", true);
        when(usuarios.buscarPorCorreo(new Email("moderadora@sendik.co"))).thenReturn(Optional.of(quien));

        GrantedModeratorsResult resultado = caso.execute(List.of("moderadora@sendik.co"));

        verify(usuarios).otorgarRol(quien.id(), Role.MODERATOR, AHORA);
        assertThat(resultado.otorgados()).containsExactly("moderadora@sendik.co");
    }

    /**
     * El correo llega como lo escribio quien configuro el entorno, y ahi cabe cualquier
     * mayuscula. Sin normalizar, `Moderadora@Sendik.co` no encontraria la cuenta y la
    ...
    void deberia_encontrar_la_cuenta_aunque_el_correo_venga_con_mayusculas() {
        ...
        caso.execute(List.of("  Moderadora@Sendik.CO  "));
    ...
    void deberia_no_crear_la_cuenta_cuando_el_correo_configurado_no_existe() {
        ...
        GrantedModeratorsResult resultado = caso.execute(List.of("fantasma@sendik.co"));

        verify(usuarios, never()).otorgarRol(any(), any(), any());
        assertThat(resultado.sinCuenta()).containsExactly("fantasma@sendik.co");
        assertThat(resultado.otorgados()).isEmpty();
    }
    ...
    void deberia_seguir_con_el_resto_despues_de_una_entrada_invalida() {
        User quien = cuenta("buena@sendik.co", true);
        when(usuarios.buscarPorCorreo(new Email("buena@sendik.co"))).thenReturn(Optional.of(quien));

        GrantedModeratorsResult resultado = caso.execute(List.of("@@@", "buena@sendik.co"));

        assertThat(resultado.otorgados()).containsExactly("buena@sendik.co");
        verify(usuarios).otorgarRol(quien.id(), Role.MODERATOR, AHORA);
    }
    ...
    void deberia_no_revocar_ningun_rol() {
        when(usuarios.buscarPorCorreo(any())).thenReturn(Optional.of(cuenta("moderadora@sendik.co", true)));
    ...
    void deberia_no_otorgar_el_rol_a_una_cuenta_sin_el_correo_verificado() {
        User quien = cuenta("sin-verificar@sendik.co", false);
        when(usuarios.buscarPorCorreo(new Email("sin-verificar@sendik.co"))).thenReturn(Optional.of(quien));

        GrantedModeratorsResult resultado = caso.execute(List.of("sin-verificar@sendik.co"));

        verify(usuarios, never()).otorgarRol(any(), any(), any());
        assertThat(resultado.sinVerificar()).containsExactly("sin-verificar@sendik.co");
        assertThat(resultado.otorgados()).isEmpty();
    }
}
