package co.sendik.shared.crypto;
...
public class AesGcmSensitiveDataCipher implements SensitiveDataCipher {
    ...
    public AesGcmSensitiveDataCipher(CryptoProperties propiedades) {
        ...
        this.claveDeBusqueda =
                new SecretKeySpec(decodificar(propiedades.lookupKey(), "sendik.crypto.lookup-key"), ALGORITMO_HMAC);

        if (!clavesDeDatos.containsKey(versionVigente)) {
            throw new IllegalStateException("sendik.crypto.current-version apunta a la version " + versionVigente
                    + ", que no esta en sendik.crypto.data-keys");
        }
    ...
    public String descifrar(EncryptedValue cifrado) {
        ...
        if (clave == null) {
            ...
            throw new IllegalStateException(
                    "No hay clave para la version " + cifrado.keyVersion() + " en sendik.crypto.data-keys");
        }
    ...
    private static Map<Integer, SecretKeySpec> leerClaves(Map<Integer, String> configuradas) {
        ...
        configuradas.forEach((version, base64) -> claves.put(
                version, new SecretKeySpec(decodificar(base64, "sendik.crypto.data-keys[" + version + "]"), "AES")));
    ...
    private void exigirClaveDeBusquedaDistinta(CryptoProperties propiedades) {
        ...
        propiedades.dataKeys().forEach((version, base64) -> {
            if (base64.trim().equals(busqueda)) {
                throw new IllegalStateException("sendik.crypto.lookup-key es igual a la clave de datos de la version "
